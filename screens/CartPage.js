...
export default function CartPage({
  ...
  const { amount, id, balance } = selectedBeneficiary;
  ...
  const handlePrintReceipt = () => {
    ...
    // const printCenteredQRCode = (data) => {
    //   const printableWidth = 384;
  
    //   const leftPadding = Math.floor((printableWidth - 200) / 2); // Assuming a QRCode size of 200 dots
  
    //   BluetoothEscposPrinter.printQRCode(data, 200, 0, leftPadding, 0, 1);
  
    // };

    // const barcodeOptions = {
    //   x: 20, // X coordinate for the barcode's starting position
    //   y: 96, // Y coordinate for the barcode's starting position
    //   type: BluetoothTscPrinter.BARCODETYPE.CODE128,
    //   height: 40, // Height of the barcode
    //   readable: 1, // Make the barcode human-readable
    //   rotation: BluetoothTscPrinter.ROTATION.ROTATION_0,
    //   code: '1234567890', // The data to encode in the barcode
    // };
    ...
    try {
      ...
      cartItems.forEach((item) => {
        BluetoothEscposPrinter.printColumn(columnWidths, 
          ...
          [item.name.toString(), item.quantity.toString(), item.price.toString(), (item.price * item.quantity).toString()], {
            encoding: 'UTF8'
          });
      });
      ...
      BluetoothEscposPrinter.printColumn([12, 8, 12], 
        ...
        ["Total", cartItems.length.toString(), totalAmount.toString()], { encoding: 'UTF8', codepage: 11,});
      BluetoothEscposPrinter.printText("\n\r", {});
